      ...
      <ul>
        {songs.map((song) => (
          <li key={song._id} className="mb-2">
            ...
            <p className="text-sm text-gray-400">Duration: {song.duration}s</p>
          </li>
        ))}
      </ul>
    </div>
...
//   const handleAddSong = async (url: string, allSongs: Song[]) => {
//     try {
...
//         <div className="absolute top-4 left-4 z-50 w-72 bg-gray-800 p-4 rounded-xl shadow-lg max-h-[80vh] overflow-y-auto">
//           <Playlist 
//               songs={allSongs} onSelectSong={(song) => setCurrentSong(song)} />
//         </div>
//       )}
...
//               isPlaying={isPlaying}
//               onMouseDown={(e) => {
//                 const track = (e.target as HTMLElement).closest(".track");
//                 if (!track) return;
